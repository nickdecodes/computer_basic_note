...
- [1 终端操作数据库](#1-终端操作数据库)
    - [1.1 登陆数据库](#11-登陆数据库)
    - [1.2 查询数据库服务器中所有的数据](#12-查询数据库服务器中所有的数据)
    - [1.3 选中某一个数据库进行操作](#13-选中某一个数据库进行操作)
    - [1.4 退出数据库服务器](#14-退出数据库服务器)
    - [1.5 在数据库服务器中创建数据库](#15-在数据库服务器中创建数据库)
    - [1.6 查看某个数据库中所有的数据表](#16-查看某个数据库中所有的数据表)
    - [1.7 创建一个数据表](#17-创建一个数据表)
    - [1.8 查看数据表是否创建成功](#18-查看数据表是否创建成功)
    - [1.9 查看创建好的数据表的结构](#19-查看创建好的数据表的结构)
    - [1.10 查看表中的记录](#110-查看表中的记录)
    - [1.11 往数据表中添加数据记录](#111-往数据表中添加数据记录)
    - [1.12 mysql常用数据类型](#112-mysql常用数据类型)
    - [1.13 数据类型如何选择](#113-数据类型如何选择)
    - [1.14 删除数据](#114-删除数据)
    - [1.15 修改数据](#115-修改数据)
    - [总结](#总结)
- [2 数据库建表约束](#2-数据库建表约束)
    ...
    - [2.6 外键约束](#26-外键约束)
- [3 数据表设计范式](#3-数据表设计范式)
    - [3.1 第一范式1NF](#31-第一范式1NF)
    - [3.2 第二范式2NF](#32-第二范式2NF)
    - [3.3 第三范式3NF](#33-第三范式3NF)
- [4 查询练习](#4-查询练习)
    - [4.1 数据准备](#41-数据准备)
    - [4.2 1到10](#42-1到10)
    - [4.3 10过程解析](#43-10过程解析)
    - [4.4 分组计算平均成绩](#44-分组计算平均成绩)
    - [4.5 分组条件与模糊查询](#45-分组条件与模糊查询)
    - [4.6 范围查询的两种方式](#46-范围查询的两种方式)
    - [4.7 多表查询](#47-多表查询)
    - [4.8 三表关联查询](#48-三表关联查询)
    - [4.9 子查询加分组求平均分](#49-子查询加分组求平均分)
    - [4.10 子查询](#410-子查询)
    - [4.11 year函数与待in关键字的子查询](#411-year函数与待in关键字的子查询)
    - [4.12 多层嵌套子查询](#412-多层嵌套子查询)
    - [4.13 多表查询](#413-多表查询)
    - [4.14 in表示或者关系](#414-in表示或者关系)
    - [4.15 where条件查询](#415-where条件查询)
    - [4.16 子查询](#416-子查询)
    - [4.17 union和notin的使用](#417-子查询)
    - [4.18 any表示至少一个desc降序](#418-any表示至少一个desc降序)
    - [4.19 all表示所有](#419-all表示所有)
    - [4.20 as取别名-union求并集](#420-as取别名-union求并集)
    - [4.21 union求并集](#421-union求并集)
    - [4.22 复制表数据做条件查询](#422-复制表数据做条件查询)
    - [4.23 子查询](#423-子查询)
    - [4.24 条件加分组筛选](#424-条件加分组筛选)
    - [4.25 notlike模糊查询取反](#425-notlike模糊查询取反)
    - [4.26 year函数与now函数](#426-year函数与now函数)
    - [4.27 max与min函数](#427-max与min函数)
    - [4.28 多字段排序](#428-多字段排序)
    - [4.29 子查询](#429-子查询)
    - [4.30 max函数与子查询](#430-子查询)
    - [4.31 子查询](#431-子查询)
    - [4.32 按等级查询](#432-按等级查询)
- [5 索引](#5-索引)
    - [5.1 普通索引](#51-普通索引)
    - [5.2 唯一索引](#52-唯一索引)
- [6 连接查询](#6-连接查询)
    ...
    - [6.2 外连接](#62-外连接)
- [7 事务](#7-事务)
    - [7.1 事务用来做什么](#71-事务用来做什么)
    - [7.2 如何开启事务](#72-如何开启事务)
    - [7.3 commit-rollback](#73-commit-rollback)
    - [7.4 手动开启事务begin-start-transaction](#74-手动开启事务begin-start-transaction)
    - [7.5 ACID特征与使用](#75-ACID特征与使用)
- [8 事务隔离性](#8-事务隔离性)
    - [8.1 脏读](#81-脏读)
    - [8.2 不可重复读](#82-不可重复读)
    - [8.3 幻读](#83-幻读)
