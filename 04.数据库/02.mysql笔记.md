...
- [终端操作数据库](#终端操作数据库)
    - [登陆数据库](#登陆数据库)
    - [查询数据库服务器中所有的数据](#查询数据库服务器中所有的数据)
    - [选中某一个数据库进行操作](#选中某一个数据库进行操作)
    - [退出数据库服务器](#退出数据库服务器)
    - [在数据库服务器中创建数据库](#在数据库服务器中创建数据库)
    - [查看某个数据库中所有的数据表](#查看某个数据库中所有的数据表)
    - [创建一个数据表](#创建一个数据表)
    - [查看数据表是否创建成功](#查看数据表是否创建成功)
    - [查看创建好的数据表的结构](#查看创建好的数据表的结构)
    - [查看表中的记录](#查看表中的记录)
    - [往数据表中添加数据记录](#往数据表中添加数据记录)
...
- [查询练习](#查询练习)
    ...
    - [多表查询](#多表查询)
    - [三表关联查询](#三表关联查询)
    - [子查询加分组求平均分](#子查询加分组求平均分)
    - [子查询](#子查询)
    - [year函数与待in关键字的子查询](#year函数与待in关键字的子查询)
    - [多层嵌套子查询](#多层嵌套子查询)
    - [多表查询](#多表查询)
    - [in表示或者关系](#in表示或者关系)
    - [where条件查询](#where条件查询)
    - [子查询](#子查询)
    - [union和notin的使用](#子查询)
    - [any表示至少一个desc降序](#any表示至少一个desc降序)
    - [all表示所有](#all表示所有)
    - [as取别名-union求并集](#as取别名-union求并集)
    - [union求并集](#union求并集)
    - [复制表数据做条件查询](#复制表数据做条件查询)
    - [子查询](#子查询)
    - [条件加分组筛选](#条件加分组筛选)
    - [notlike模糊查询取反](#notlike模糊查询取反)
    - [year函数与now函数](#year函数与now函数)
    - [max与min函数](#max与min函数)
    - [多字段排序](#多字段排序)
    - [子查询](#子查询)
    - [max函数与子查询](#子查询)
...
- [事务](#事务)
    ...
    - [手动开启事务begin-start-transaction](#手动开启事务begin-start-transaction)
    - [ACID特征与使用](#ACID特征与使用)
- [事务隔离性](#事务隔离性)
    - [脏读](#脏读)
    - [不可重复读](#不可重复读)
